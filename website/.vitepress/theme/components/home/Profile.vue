      ...
      <div class="flex flex-col gap-8">
        <div class="flex gap-8">
          ...
          <div>
            ...
            <h2 class="-ml-2 text-7xl sm:text-8xl max-w-sm lg:max-w-none">{{ frontmatter.profile.title }}</h2>
          </div>
        </div>
        <p v-for="paragraph in frontmatter.profile.text" :key="paragraph" class="max-w-2xl text-xl">
          {{ paragraph }}
        </p>
